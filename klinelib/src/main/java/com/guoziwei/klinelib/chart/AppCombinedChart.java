...
public class AppCombinedChart extends CombinedChart {
    ...
    private IMarker mBubbleMarker;

    private float mYCenter=0;
    private Context mContext;
    private VolumeProfileListener profileListener;
    ...
    public void setmBubbleMarker(IMarker marker) {
        this.mBubbleMarker = marker;
...
//            mMarker.refreshContent(e, highlight);
            ...
            if (mBubbleMarker != null) {
                mBubbleMarker.refreshContent(e, highlight);
            }
...
//            if (mMarker instanceof LineChartYMarkerView) {
            ...
            MyMarkerView markerView = (MyMarkerView) mBubbleMarker;


            int bottom = DisplayUtils.dip2px(mContext, 20);
//            mMarker.draw(canvas, mViewPortHandler.contentRight(), pos[1] - yMarker.getMeasuredHeight() / 2);
            //下面的时间线
            mXMarker.draw(canvas, pos[0] - (xMarker.getMeasuredWidth() / 2), getMeasuredHeight() - bottom);
            //k线短评
            if (markerView != null && e.isDrawMarker()) {
...
//            }
    ...
    protected void autoScale() {
        ...
        if (profileListener!=null && mData.getCandleData()!=null) {
            profileListener.VisibleXAndY((int) (fromX),(int)(toX));
        }
    }
    ...
    public interface VolumeProfileListener{
        void VisibleXAndY(int fromX, int toX);
    }

    public void setProfileListener(VolumeProfileListener profileListener) {
