...
import com.hyphenate.easeui.net.HXUserInfo;
import com.hyphenate.easeui.net.UrlConfig;
import com.hyphenate.easeui.net.UserInfoService;
...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        if (userAvatarView != null) {
            ...
            Call<HXUserInfo> newsData = userInfoService.getNewsData(UserId);
            newsData.enqueue(new Callback<HXUserInfo>() {
                @Override
                public void onResponse(Call<HXUserInfo> call, Response<HXUserInfo> response) {
                    if (response != null
                            && response.body() != null
                            && response.body().getData() != null
                            && response.body().getData().getResult() != null) {
                        if (message.direct() == Direct.SEND) {
                            String avatar = response.body().getData().getResult().getHeadImage() + "";
                            message.setAttribute("avatar", avatar);
                            EaseUserUtils.setUserAvatar(context, message, userAvatarView);
                        } else {
                            String username = response.body().getData().getResult().getNickName() + "";
                            String avatar = response.body().getData().getResult().getHeadImage() + "";

                            message.setAttribute("avatar", avatar);
                            message.setAttribute("username", username);
                            EaseUserUtils.setUserNick(message, usernickView);
                            EaseUserUtils.setUserAvatar(context, message, userAvatarView);
                        }
                    } else {
                        Toast.makeText(context, "获取聊天信息失败", Toast.LENGTH_SHORT).show();
                    }

                }

                @Override
                public void onFailure(Call<HXUserInfo> call, Throwable t) {
    ...
    public void getUserInfo(Direct direct, String UserId) {
