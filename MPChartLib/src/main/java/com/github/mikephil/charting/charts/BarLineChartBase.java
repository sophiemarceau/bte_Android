...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    protected void drawGridBackground(Canvas c) {
        ...
        if (mDrawBorders) {
            ...
            float top=contentRect.top;
            //卸载offset（）中的偏移量
            float v = Utils.convertDpToPixel(50);
//            c.drawRect(left,0,right,bottom, mBorderPaint);
            mBorderPaint.setAlpha(99);
            //绘制下边界
            c.drawLine(left, bottom, right+v , bottom, mBorderPaint);
            //绘制竖直的
            c.drawLine(right,0,right,bottom,mBorderPaint);
